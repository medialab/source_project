...
d3.json(apiUrl, function(error, data) {
  ...
  svg.append('g').selectAll('relations')
    .data(_.filter(graph.relations,function(r) { return r.source.typeId === 4 && r.target.typeId === 4;}))
    .enter()
    .append('polyline')
    .attr('class','relations')
    .attr('stroke', function(d){
      var s = _.find(graph.organisations, 'recordId', d.source.id);
      if(s) return d3.select("#record"+d.source.id).attr("fill");
    })
      ...
      // console.log(d, "#record"+d.source.id, "s",s,"t",t);
        if( s && t ){
          ...
          return p.sx +"," + (p.sy + moveY)  + " "
              +  (p.sx + 30 + moveX) +"," + (p.sy + moveY) + " "
              +  (p.sx + 30 + moveX) +"," + (p.ty + moveY) + " "
              +  p.tx +"," + (p.ty + moveY);
